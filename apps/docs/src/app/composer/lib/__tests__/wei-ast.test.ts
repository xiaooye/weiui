...
describe("framework-neutral Composer AST", () => {
  ...
  it.each(["react", "vue", "solid", "svelte", "elements"] as const)("generates %s from the same semantic node", (target) => { const code = generateWeiRuntimeCode(tree, target); expect(code).toContain(target === "elements" ? "civ-button" : "Button"); });
