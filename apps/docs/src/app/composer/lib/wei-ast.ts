import { getComponentMetadata, type WeiFramework } from "@weiui/core/registry";
...
function renderJsx(node: WeiNode, depth: number, target: "react"|"solid"): string { const i="  ".repeat(depth); if(node.type==="text")return i+node.value;if(!supported(node.component,target))return `${i}{/* ${node.component} is not available in @weiui/${target}. */}`;const a=propsJsx(node.props),o=`<${node.component}${a?` ${a}`:""}`;if(!node.children.length)return `${i}${o} />`;return `${i}${o}>\n${node.children.map(c=>renderJsx(c,depth+1,target)).join("\n")}\n${i}</${node.component}>`; }
function renderVue(node: WeiNode, depth: number): string { const i="  ".repeat(depth);if(node.type==="text")return i+node.value;if(!supported(node.component,"vue"))return `${i}<!-- ${node.component} is not available in @weiui/vue. -->`;const a=propsVue(node.props),o=`<${node.component}${a?` ${a}`:""}`;if(!node.children.length)return `${i}${o} />`;return `${i}${o}>\n${node.children.map(c=>renderVue(c,depth+1)).join("\n")}\n${i}</${node.component}>`; }
function renderSvelte(node: WeiNode, depth: number): string { const i="  ".repeat(depth);if(node.type==="text")return i+node.value;if(!supported(node.component,"svelte"))return `${i}<!-- ${node.component} is not available in @weiui/svelte. -->`;const a=propsSvelte(node.props),o=`<${node.component}${a?` ${a}`:""}`;if(!node.children.length)return `${i}${o} />`;return `${i}${o}>\n${node.children.map(c=>renderSvelte(c,depth+1)).join("\n")}\n${i}</${node.component}>`; }
function renderElements(node: WeiNode, depth: number): string { const i="  ".repeat(depth);if(node.type==="text")return i+esc(node.value);if(!supported(node.component,"elements"))return `${i}<!-- ${node.component}: no WeiUI Custom Element distribution. -->`;const tag=`wui-${kebab(node.component)}`;const a=Object.entries(node.props).filter(([,v])=>["string","number","boolean"].includes(typeof v)&&v!==false&&v!=="").map(([k,v])=>typeof v==="boolean"?kebab(k):`${kebab(k)}=${JSON.stringify(String(v))}`).join(" ");return `${i}<${tag}${a?` ${a}`:""}>${node.children.length?`\n${node.children.map(c=>renderElements(c,depth+1)).join("\n")}\n${i}`:""}</${tag}>`; }
...
export function generateWeiRuntimeCode(nodes: readonly ComponentNode[], target: WeiTarget): string { const ast=toWeiAst(nodes),names=[...collect(ast,target)].sort();if(target==="elements"){return `<!-- WeiUI Elements: explicit registration, no import side effects. -->\n<script type="module">\n  import { defineAll } from "@weiui/elements";\n  import "@weiui/css";\n  defineAll();\n</script>\n${ast.map(n=>renderElements(n,1)).join("\n")}`;}const pkg=`@weiui/${target}`;if(target==="vue")return `<script setup lang="ts">\nimport { ${names.join(", ")} } from "${pkg}";\nimport "@weiui/css";\n</script>\n\n<template>\n${ast.map(n=>renderVue(n,1)).join("\n")}\n</template>`;if(target==="svelte")return `<script lang="ts">\n  import { ${names.join(", ")} } from "${pkg}";\n  import "@weiui/css";\n</script>\n\n${ast.map(n=>renderSvelte(n,0)).join("\n")}`;const body=ast.map(n=>renderJsx(n,2,target)).join("\n");return `import { ${names.join(", ")} } from "${pkg}";\nimport "@weiui/css";\n\nexport default function Composition() {\n  return (\n    <>\n${body}\n    </>\n  );\n}`; }
