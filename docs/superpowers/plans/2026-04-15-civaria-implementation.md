# Civaria Design System — Full Implementation Plan
...
### Task 2: @civaria/tokens — Package + Token Source Files
...
### Task 3: @civaria/tokens — Build Pipeline
...
export function generateTs(tokens: FlatToken[]): string {
  const lines: string[] = [
    "// Auto-generated by @civaria/tokens — do not edit",
    "",
  ];
...
Expected: CSS file with `@layer civ-tokens { :root { --civ-color-neutral-0: oklch(1 0 0); ...`
...
### Task 4: @civaria/a11y — Contrast Validator
...
Expected: `.dark { --civ-color-background: oklch(0.145 0.010 240); ...`
...
### Task 7: @civaria/css — Package + Reset + Base + A11y Styles
...
```json
{
  ...
  "exports": {
    ".": "./dist/civaria.css",
    "./civaria.css": "./dist/civaria.css",
    "./reset.css": "./dist/reset.css",
    "./elements/*": "./dist/elements/*"
  },
  "files": ["dist"],
  "scripts": {
    "build": "postcss src/index.css -o dist/civaria.css && lightningcss --minify dist/civaria.css -o dist/civaria.min.css"
  },
  "devDependencies": {
    "postcss": "^8.5",
    "postcss-cli": "^11.0",
...
@layer civ-base {
  ...
  h1, h2, h3, h4, h5, h6 {
    ...
    line-height: var(--civ-font-lineHeight-tight);
    letter-spacing: var(--civ-font-letterSpacing-tight);
  }
  code, kbd, pre { font-family: var(--civ-font-family-mono); }
}
```
...
### Task 8: @civaria/css — Button Element
...
@layer civ-elements {
  ...
  .civ-button--solid { background-color: var(--civ-color-primary); color: var(--civ-color-primary-foreground); }
  .civ-button--solid:hover { filter: brightness(1.1); }
  .civ-button--solid:active { filter: brightness(0.95); transform: scale(0.98); }
  ...
  .civ-button--soft { background-color: oklch(from var(--civ-color-primary) l c h / 0.1); color: var(--civ-color-primary); }
  .civ-button--link { background-color: transparent; color: var(--civ-color-primary); text-decoration: underline; text-underline-offset: 4px; padding: 0; min-block-size: 0; min-inline-size: 0; }

  .civ-button--destructive.civ-button--solid { background-color: var(--civ-color-destructive); color: var(--civ-color-destructive-foreground); }
  .civ-button--destructive.civ-button--outline { border-color: var(--civ-color-destructive); color: var(--civ-color-destructive); }

  .civ-button[data-disabled] { opacity: 0.5; cursor: not-allowed; pointer-events: none; }
  .civ-button[data-loading] { position: relative; cursor: wait; }

  @media (prefers-reduced-motion: no-preference) {
    .civ-button { transition: background-color, border-color, color, filter, transform; transition-duration: var(--civ-motion-duration-fast); transition-timing-function: var(--civ-motion-easing-default); }
  }
}
```
...
### Task 9: @civaria/css — Input Element
...
@layer civ-elements {
  ...
  .civ-input:hover { border-color: var(--civ-color-primary); }
  .civ-input:focus-within { border-color: var(--civ-color-ring); outline: 2px solid var(--civ-color-ring); outline-offset: 0; }
  .civ-input[data-invalid], .civ-input:invalid { border-color: var(--civ-color-destructive); }
  .civ-input[data-disabled], .civ-input:disabled { opacity: 0.5; cursor: not-allowed; background-color: var(--civ-color-muted); }
  ...
  @media (prefers-reduced-motion: no-preference) {
    .civ-input { transition: border-color, outline-color; transition-duration: var(--civ-motion-duration-fast); }
  }
}
```
...
### Task 10: @civaria/css — Badge, Card, Avatar, Skeleton
...
Each element follows the same pattern: `@layer civ-elements`, `civ-` prefix, BEM naming, data-attribute states, logical properties, motion-safe animations. See spec sections 14.2 (naming) and 20 (state matrix).

- [ ] **Step 1: Create badge.css** — solid/soft/outline variants, destructive/success/warning colors, full-radius pill shape.

- [ ] **Step 2: Create card.css** — header/content/footer compound, `container-type: inline-size`, container query for small widths, `civ-shadow-sm`.
...
### Task 11: @civaria/cli — Package + Commands
...
- [ ] **Step 3: Implement init command** — creates `civaria.config.json` with token paths and output config, prints next steps.

- [ ] **Step 4: Implement tokens build/validate commands** — uses `execFileSync("pnpm", ["--filter", "@civaria/tokens", "build"])` to delegate to the token package safely (no shell injection).
...
- [ ] **Step 1: Create Next.js 16 app** with MDX support, `@civaria/css` and `@civaria/tokens` as workspace deps.

- [ ] **Step 2: Create root layout** importing `@civaria/tokens/tokens.css` and `@civaria/css/civaria.css`.

- [ ] **Step 3: Create landing page** with heading, description, and two Civaria button links (Get Started, Components).
...
### Task 13: @civaria/headless — Package Scaffold + Core Utilities
...
### Task 23: civaria — Package Scaffold + CivariaProvider

Package scaffold with `@civaria/headless` and `@civaria/css` deps, `tailwind-variants` and `tailwind-merge` deps. `CivariaProvider` context with locale object. `cn()` utility wrapping `twMerge`.

**Commit:** `feat(react): scaffold React package with CivariaProvider and cn utility`
...
Visual theme builder at `/themes`. Pick primary color → generate full AAA-compliant token set → preview all components live → export as CSS or JSON. Uses `@civaria/a11y` to show pass/fail contrast badges for each generated pair.
