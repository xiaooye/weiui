...
**Tech Stack:** Next.js 15 App Router, React 19, `civaria` + subpath exports (editor/data-table/chart), HTML5 native drag-and-drop (no `react-dnd`), `useSyncExternalStore` for localStorage, Shiki for code highlight (already in docs), Vitest, Playwright.
...
- [ ] Run: `pnpm --filter @civaria/docs test -- prop-controls`
...
```tsx
"use client";
import { Switch, Field } from "civaria";
...
```tsx
"use client";
import { ColorPicker, Field } from "civaria";
...
```tsx
"use client";
import { Field, Textarea } from "civaria";
...
- [ ] `pnpm --filter @civaria/docs test -- prop-controls` — PASS.
- [ ] `pnpm --filter @civaria/docs build` — clean.
...
export function generateCode(input: CodeGenInput, opts: CodeGenOptions): string {
  if (opts.target === "html") {
    // Simple HTML form: use civ-* classes. (Detailed mapping belongs in Task 7; stub here.)
...
const heavyLazy: Record<string, ReturnType<typeof lazy>> = {
  Editor: lazy(() => import("civaria/editor").then((m) => ({ default: m.Editor as any }))),
  DataTable: lazy(() => import("civaria/data-table").then((m) => ({ default: m.DataTable as any }))),
  BarChart: lazy(() => import("civaria/chart").then((m) => ({ default: m.BarChart as any }))),
  LineChart: lazy(() => import("civaria/chart").then((m) => ({ default: m.LineChart as any }))),
  AreaChart: lazy(() => import("civaria/chart").then((m) => ({ default: m.AreaChart as any }))),
  PieChart: lazy(() => import("civaria/chart").then((m) => ({ default: m.PieChart as any }))),
  DonutChart: lazy(() => import("civaria/chart").then((m) => ({ default: m.DonutChart as any }))),
  RadarChart: lazy(() => import("civaria/chart").then((m) => ({ default: m.RadarChart as any }))),
};
...
export function PlaygroundPreview({ component, props }: PlaygroundPreviewProps) {
  return <div className="civ-playground__stage">{renderComponent(component, props)}</div>;
}
```
...
"use client";
import { useState } from "react";
import { Button, toast } from "civaria";
...
export function CodeOutput({ schema, props }: CodeOutputProps) {
  ...
  return (
    <div className="civ-playground__code">
      <div className="civ-playground__code-tabs">
      ...
      <pre className="civ-playground__code-pre"><code>{code}</code></pre>
    </div>
  );
}
```
...
- [ ] `pnpm --filter @civaria/docs test` — PASS.
- [ ] `pnpm --filter @civaria/docs build` — PASS.
...
export default function PlaygroundPage() {
  ...
  return (
    <main className="civ-playground">
      <ComponentSelector
        selected={state.component}
        onSelect={(c) => setState({ component: c, props: {} })}
      />
      <div className="civ-playground__main">
...
- [ ] `pnpm --filter @civaria/docs test -- playground`
...
(Detailed code omitted for brevity — follow Task 3's style. The ComponentSelector becomes a `<Stack>` of category accordions using Civaria `Accordion`; theme uses a `ToggleGroup`; viewport uses a `ToggleGroup`.)
...
- Exposes `<SelectionOutline rect={rect} />` that draws 1px solid `var(--civ-color-primary)` with `pointer-events: none`.
...
export function WysiwygCanvas({ tree, selectedId, onSelect, viewport }: WysiwygCanvasProps) {
  ...
  return (
    <div className="civ-composer__canvas">
      <div className="civ-composer__stage" ref={stageRef} style={{ maxInlineSize: maxSize }}>
...
Above the canvas: Civaria `ToggleGroup` with values `375 | 768 | 1024 | 1280 | full`. State stored in `page.tsx` + serialized into localStorage along with the tree.
...
export function DropZones({ rect, isContainer, onDrop }: {
  ...
  return (
    <>
      <div className="civ-composer__drop civ-composer__drop--top"    style={zoneStyle(rect, "top")}    onDragOver={(e) => e.preventDefault()} onDrop={handle("top")} />
      <div className="civ-composer__drop civ-composer__drop--right"  style={zoneStyle(rect, "right")}  onDragOver={(e) => e.preventDefault()} onDrop={handle("right")} />
      <div className="civ-composer__drop civ-composer__drop--bottom" style={zoneStyle(rect, "bottom")} onDragOver={(e) => e.preventDefault()} onDrop={handle("bottom")} />
      <div className="civ-composer__drop civ-composer__drop--left"   style={zoneStyle(rect, "left")}   onDragOver={(e) => e.preventDefault()} onDrop={handle("left")} />
      {isContainer && (
        <div className="civ-composer__drop civ-composer__drop--center" style={zoneStyle(rect, "center")} onDragOver={(e) => e.preventDefault()} onDrop={handle("center")} />
      )}
    </>
  );
}
...
Palette items set `dataTransfer.setData("application/x-civaria-node", JSON.stringify(makeNode(type)))` on drag start. Existing canvas nodes set the same payload + `dataTransfer.setData("application/x-civaria-move", nodeId)` to distinguish.
...
- During drag: all drop zones render a `color-mix(in oklch, var(--civ-color-primary) 8%, transparent)` background.
- The zone currently under the pointer: `var(--civ-color-primary)` 20% with a 3px primary-colored border on the corresponding edge.
...
export function LayoutChips({ node, position, onUpdate }: { node: ComponentNode; position: { x: number; y: number }; onUpdate: (props: Record<string, unknown>) => void }) {
  if (node.type === "Stack") {
    return (
      <div className="civ-composer__chips" style={{ left: position.x, top: position.y }}>
        <ToggleGroup
          type="single"
...
- **HTML**: full `<!DOCTYPE html>` document with CDN `<link>` for tokens + CSS; tree rendered using `civ-*` classes on plain HTML tags (e.g., `<button class="civ-button civ-button--solid">`); Stack becomes `<div class="civ-stack civ-stack--row">`; Grid becomes `<div class="civ-grid">` with inline `style="--civ-grid-cols: 3"`.
  ...
  const files = {
    "package.json": {
      content: JSON.stringify({
        ...
        dependencies: { "civaria": "latest", "@civaria/css": "latest", "@civaria/tokens": "latest", react: "^19.0.0", "react-dom": "^19.0.0" },
      }),
    },
    "src/App.tsx": { content: code },
    "src/index.tsx": { content: `import "@civaria/tokens/tokens.css";\nimport "@civaria/css";\nimport { createRoot } from "react-dom/client";\nimport App from "./App";\ncreateRoot(document.getElementById("root")!).render(<App />);` },
...
- `playground.spec.ts`: navigate to `/playground`, select Dialog from sidebar, toggle `size` to `lg`, copy code, verify clipboard contains `import { Dialog } from "civaria"` and `size="lg"`.
...
pnpm test
pnpm --filter @civaria/docs build
pnpm --filter @civaria/docs exec playwright test
```
