...
function visual(name, tag = "div") {
  ...
  return defineComponent({ name: `Civ${name}`, inheritAttrs: false, props: { variant: String, size: String }, setup(props, { attrs, slots }) {
    return () => h(tag, { ...attrs, ...semanticPart(component, "root", { variant: props.variant, size: props.size }), class: [cls(component, "root"), props.variant && `civ-${component}--${props.variant}`, props.size && `civ-${component}--${props.size}`, attrs.class] }, slots.default?.());
  }});
}
...
export const Accordion = defineComponent({
  name: "CivAccordion", props: { ...itemProps, type: { type: String, default: "single" }, modelValue: Array, defaultValue: { type: Array, default: () => [] } }, emits: ["update:modelValue"],
  setup(props, { emit }) {
    const controller = createAccordionController({ id: props.id ?? "civ-accordion", type: props.type, defaultValue: props.modelValue ?? props.defaultValue, onValueChange: value => emit("update:modelValue", [...value]) });
...
function disclosure(name, createController, renderContent) {
  ...
  return defineComponent({
    ...
    setup(props, { slots, emit }) {
      ...
      const controller = createController({ id: props.id ?? `civ-${component}`, defaultOpen: props.modelValue ?? props.defaultOpen, onOpenChange: value => emit("update:modelValue", value) });
...
export const Menu = defineComponent({
  name: "CivMenu", props: { ...itemProps, trigger: { type: String, default: "Menu" } }, emits: ["select"],
  setup(props, { emit }) {
    const controller = createMenuController({ id: props.id ?? "civ-menu" }); const revision = bind(controller); controller.setItems(props.items); watch(() => props.items, value => controller.setItems(value));
...
export const Select = defineComponent({
  name: "CivSelect", props: { ...itemProps, modelValue: String, defaultValue: { type: String, default: "" }, placeholder: { type: String, default: "Select…" }, name: String, required: Boolean, disabled: Boolean }, emits: ["update:modelValue"],
  setup(props, { emit }) {
    const controller = createSelectController({ id: props.id ?? "civ-select", defaultValue: props.modelValue ?? props.defaultValue, onValueChange: value => emit("update:modelValue", value) });
...
export const Tabs = defineComponent({
  name: "CivTabs", props: { ...itemProps, modelValue: String, defaultValue: { type: String, default: "" }, orientation: { type: String, default: "horizontal" } }, emits: ["update:modelValue"],
  setup(props, { emit }) {
    const controller = createTabsController({ id: props.id ?? "civ-tabs", defaultValue: props.modelValue ?? props.defaultValue, orientation: props.orientation, onValueChange: value => emit("update:modelValue", value) }); const revision = bind(controller); watch(() => props.modelValue, value => { if (value !== undefined) controller.syncValue(value); });
...
export const Tooltip = defineComponent({
  name: "CivTooltip", props: { id: String, content: { type: String, required: true }, delay: { type: Number, default: 0 } },
  setup(props, { slots }) { const controller = createTooltipController({ id: props.id ?? "civ-tooltip", delay: props.delay }); const revision = bind(controller); return () => { revision.value; return coreNode("span", "tooltip", "root", controller.getRootProps(), [coreNode("span", "tooltip", "trigger", controller.getTriggerProps(), slots.default?.(), { tabindex: 0 }), coreNode("span", "tooltip", "content", controller.getContentProps(), props.content)]); }; },
});

export const Combobox = defineComponent({
  name: "CivCombobox", props: { ...itemProps, modelValue: String, defaultValue: { type: String, default: "" }, inputValue: String, placeholder: String, name: String, required: Boolean, disabled: Boolean }, emits: ["update:modelValue", "update:inputValue"],
  setup(props, { emit }) {
    const controller = createComboboxController({ id: props.id ?? "civ-combobox", defaultValue: props.modelValue ?? props.defaultValue, defaultInputValue: props.inputValue ?? "", onValueChange: value => emit("update:modelValue", value), onInputValueChange: value => emit("update:inputValue", value) });
...
function choice(name, createController) {
  const component = name.toLowerCase();
  return defineComponent({
    name: `Civ${name}`, props: { id: String, modelValue: Boolean, defaultChecked: Boolean, disabled: Boolean, required: Boolean, label: String, name: String, value: { type: String, default: "on" } }, emits: ["update:modelValue"],
    setup(props, { emit }) {
      const controller = createController({ id: props.id ?? `civ-${component}`, defaultChecked: props.modelValue ?? props.defaultChecked, disabled: props.disabled, onCheckedChange: value => emit("update:modelValue", value) }); const revision = bind(controller); watch(() => props.modelValue, value => { if (value !== undefined) controller.syncChecked(value); });
