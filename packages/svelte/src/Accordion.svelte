<script lang="ts">
  import { createAccordionController } from "@weiui/core"; import { normalizeSvelteProps } from "./internal.js";
  type Item={value:string;label?:string;content?:string;disabled?:boolean};
  let {items=[],value=$bindable<string[]>([]),type="single",id="wui-accordion"}=$props<{items?:Item[];value?:string[];type?:"single"|"multiple";id?:string}>();
  ...
  $effect(()=>{if(value.length!==state.expanded.size||value.some(v=>!state.expanded.has(v)))controller.store.setState({expanded:new Set(value)})});
...
<div {...p("root")} class="wui-accordion">{#each items as item (item.value)}<div {...p("item",item)} class="wui-accordion__item"><button {...p("trigger",item)} class="wui-accordion__trigger">{item.label??item.value}</button><div {...p("content",item)} class="wui-accordion__content">{item.content??""}</div></div>{/each}</div>
