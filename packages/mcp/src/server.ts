...
export function createServer(options:CreateServerOptions={}):Server{const loadIndex=defaultLoadIndex(options.registryDir),loadComponent=defaultLoadComponent(options.registryDir);const server=new Server({name:"@weiui/mcp",version:"0.1.0"},{capabilities:{tools:{}}});server.setRequestHandler(ListToolsRequestSchema,async()=>({tools:[
{name:"list_components",description:"List WeiUI components with framework-neutral portability/anatomy metadata. Filter by category or native runtime.",inputSchema:{type:"object",properties:{category:{type:"string"},framework:{type:"string",enum:["react","vue","solid","svelte","elements"]}}}},
{name:"get_component",description:"Return docs schema plus canonical WeiUI portability, anatomy and supported runtimes.",inputSchema:{type:"object",required:["name"],properties:{name:{type:"string"}}}},
...
{name:"get_example",description:"Return a native WeiUI example for React, Vue, Solid, Svelte or Elements when supported.",inputSchema:{type:"object",required:["name"],properties:{name:{type:"string"},variant:{type:"string"},framework:{type:"string",enum:["react","vue","solid","svelte","elements"]}}}},
{name:"check_usage",description:"Lint a TSX snippet for common WeiUI mistakes.",inputSchema:{type:"object",required:["code"],properties:{code:{type:"string"}}}}
...
export async function run():Promise<void>{const server=createServer(),transport=new StdioServerTransport();await server.connect(transport);console.error("[@weiui/mcp] ready — awaiting stdio messages")}
