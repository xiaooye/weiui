...
import { getComponentMetadata, type PortabilityClass, type WeiFramework } from "@civaria/core/registry";
...
const REMOTE_BASE="https://civaria.dev/registry";
...
function defaultRegistryDir():string{if(process.env.CIVARIA_MCP_REGISTRY_DIR)return process.env.CIVARIA_MCP_REGISTRY_DIR;const here=dirname(fileURLToPath(import.meta.url));return resolve(here,"..","registry")}
...
async function fetchRemoteJson<T>(url:string):Promise<T>{const res=await fetch(url);if(!res.ok)throw new Error(`[@civaria/mcp] failed to fetch ${url}: ${res.status} ${res.statusText}`);return await res.json() as T}
