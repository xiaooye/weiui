...
export interface DisclosureController extends Controller<DisclosureState> {
  open(): void; close(): void; toggle(): void; setOpen(open: boolean): void; syncOpen(open: boolean): void;
...
export function createDisclosureController(options: DisclosureOptions): DisclosureController {
  ...
  const syncOpen = (open: boolean) => { if (store.getState().open !== open) store.setState({ open }); };
  const setOpen = (open: boolean) => { if (store.getState().open === open) return; syncOpen(open); options.onOpenChange?.(open); };
  return {
    ...baseController(store), open: () => setOpen(true), close: () => setOpen(false), toggle: () => setOpen(!store.getState().open), setOpen, syncOpen,
...
export interface AccordionController extends Controller<AccordionState> {
  toggle(value: string): void; isExpanded(value: string): boolean; syncValue(value: readonly string[]): void;
...
export function createAccordionController(options: AccordionOptions): AccordionController {
  ...
  const syncValue = (value: readonly string[]) => store.setState({ expanded: new Set(type === "single" ? value.slice(-1) : value) });
  ...
  return {
    ...baseController(store), toggle, isExpanded, syncValue,
...
export interface TabsController extends Controller<TabsState> { select(value: string): void; syncValue(value: string): void; getRootProps(): WeiDOMProps; getListProps(): WeiDOMProps; getTriggerProps(value: string, disabled?: boolean): WeiDOMProps; getContentProps(value: string): WeiDOMProps; }
export function createTabsController(options: TabsOptions): TabsController {
  ...
  const syncValue = (value: string) => { if (value !== store.getState().value) store.setState({ value }); };
  const select = (value: string) => { if (value === store.getState().value) return; syncValue(value); options.onValueChange?.(value); };
  return {
    ...baseController(store), select, syncValue,
...
export interface ChoiceController extends Controller<ChoiceState> { toggle(): void; setChecked(checked: boolean): void; syncChecked(checked: boolean): void; getRootProps(): WeiDOMProps; getControlProps(): WeiDOMProps; getHiddenInputProps(name?: string, value?: string, required?: boolean): WeiDOMProps; }
export function createChoiceController(options: ChoiceOptions): ChoiceController {
  ...
  const syncChecked = (checked: boolean) => { if (checked !== store.getState().checked) store.setState({ checked }); };
  const setChecked = (checked: boolean) => { if (options.disabled || checked === store.getState().checked) return; syncChecked(checked); options.onCheckedChange?.(checked); }; const state = () => store.getState().checked ? "checked" : "unchecked";
  return {
    ...baseController(store), toggle: () => setChecked(!store.getState().checked), setChecked, syncChecked,
    ...
    getHiddenInputProps: (name, value = "on", required = false) => ({ attributes: { ...semanticPart(options.component, "hiddenInput"), type: "checkbox", name, value, checked: store.getState().checked, disabled: options.disabled, required, tabindex: -1, "aria-hidden": true } }),
  };
}
...
export interface SelectController extends Controller<SelectState> { readonly collection: CollectionStore; setItems(items: readonly CollectionItem[]): void; open(): void; close(): void; syncOpen(open: boolean): void; select(value: string): void; syncValue(value: string): void; highlight(index: number): void; getRootProps(): WeiDOMProps; getTriggerProps(): WeiDOMProps; getContentProps(): WeiDOMProps; getItemProps(index: number): WeiDOMProps; getHiddenInputProps(name?: string, required?: boolean, disabled?: boolean): WeiDOMProps; }
export function createSelectController(options: SelectOptions): SelectController {
  ...
  const syncOpen = (open: boolean) => store.setState({ ...store.getState(), open, highlightedIndex: open ? store.getState().highlightedIndex : -1 });
  const setOpen = (open: boolean) => { if (open === store.getState().open) return; syncOpen(open); options.onOpenChange?.(open); };
  ...
  return {
    ...
    getHiddenInputProps: (name, required = false, disabled = false) => createHiddenFormControl({ component: "select", name, value: store.getState().value, required, disabled }),
  };
}
...
export interface ComboboxController extends Controller<ComboboxState> { readonly collection: CollectionStore; setItems(items: readonly CollectionItem[]): void; open(): void; close(): void; syncOpen(open: boolean): void; select(value: string): void; syncValue(value: string): void; setInputValue(value: string): void; syncInputValue(value: string): void; highlight(index: number): void; getRootProps(): WeiDOMProps; getInputProps(): WeiDOMProps; getTriggerProps(): WeiDOMProps; getContentProps(): WeiDOMProps; getItemProps(index: number): WeiDOMProps; getHiddenInputProps(name?: string, required?: boolean, disabled?: boolean): WeiDOMProps; }
export function createComboboxController(options: ComboboxOptions): ComboboxController {
  ...
  const syncValue = (value: string) => { if (value !== store.getState().value) store.setState({ ...store.getState(), value }); };
  const syncInputValue = (inputValue: string) => { if (inputValue !== store.getState().inputValue) store.setState({ ...store.getState(), inputValue }); };
  ...
  return {
    ...
    getHiddenInputProps: (name, required = false, disabled = false) => createHiddenFormControl({ component: "combobox", name, value: store.getState().value, required, disabled }),
  };
}
...
export interface TooltipController extends Controller<TooltipState> { open(): void; close(): void; syncOpen(open: boolean): void; getRootProps(): WeiDOMProps; getTriggerProps(): WeiDOMProps; getContentProps(): WeiDOMProps; }
