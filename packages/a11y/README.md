...
Accessibility validation utilities used by `@civaria/tokens` at build time and available to apps for runtime contrast checks.

## Install

```bash
pnpm add @civaria/a11y
```
...
```ts
import { validateContrastAAA } from "@civaria/a11y";
