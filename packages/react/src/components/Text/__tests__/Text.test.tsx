...
describe("Text", () => {
  ...
  it("applies text-sm for size='sm'", () => {
    ...
    expect(screen.getByText("Small").className).toContain("civ-text--sm");
  });

  it("applies text-xl for size='xl'", () => {
    render(<Text size="xl">XL</Text>);
    expect(screen.getByText("XL").className).toContain("civ-text--xl");
  });

  it("applies muted color class", () => {
    render(<Text color="muted">Muted</Text>);
    expect(screen.getByText("Muted").className).toContain("civ-text--muted");
  });

  it("applies primary color class", () => {
    render(<Text color="primary">Primary</Text>);
    expect(screen.getByText("Primary").className).toContain("civ-text--primary");
  });

  it("applies destructive color class", () => {
    render(<Text color="destructive">Error</Text>);
    expect(screen.getByText("Error").className).toContain("civ-text--destructive");
  });

  it("applies success color class", () => {
    render(<Text color="success">OK</Text>);
    expect(screen.getByText("OK").className).toContain("civ-text--success");
  });

  it("applies weight class when specified", () => {
    render(<Text weight="bold">Bold</Text>);
    expect(screen.getByText("Bold").className).toContain("civ-text--weight-bold");
  });
